...
public class VendorRegistration extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            vdao.setEmail(email);
            vdao.setMobile(mobileNo);
            vdao.setStatus("Block");
            vdao.setType("Bus");
            VendorDTO vdto=new VendorDTO();
            boolean b=vdto.vendorRegistration(vdao);
            if(b){
                System.out.println("Successfull Person but Block");
                ...
                if(CheckDAta.checkData("vendor", "mobile", vdao.getMobile())){
                    System.out.println("Mobile is duplicate");
                    ch.setMobile("pika");
                }
                HttpSession error=request.getSession();
                error.setAttribute("error", ch);
                System.out.println("Koi na");
                response.sendRedirect("VendorView/VendorRegistration.jsp");
            }
